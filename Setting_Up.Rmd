...
Following Suyama et al. I use `fastx_timmer` to trim off the first 14 bases from Read 2. - fastq_trimmer.sh, run from the `original` directory

```{unix}


#trim the first 14 bases (the msat + anchor sequences of PCR1 primers) from Read 2, and the last 14 bases from Read 1
for i in *R2*.fastq

        do
          	cat $i | fastx_trimmer -Q33 -f 15 -o ../2trimmed/$(basename $i .fastq)_trim.fastq
        done

for i in *R1*.fastq
        do
          	cat $i | fastx_trimmer -Q33 -t 14 -o ../2trimmed/$(basename $i .fastq)_trim.fastq
...
Trim bases less than Q30, and remove sequences with less than 20 percent at Q30. I am using a 50% lower threshold than Suyama, because my reads were twice as long (150bp vs 75bp).
...
for i in *.fastq
        ...
        do
          	cat $i | fastq_quality_filter -Q33 -q 30 -p 20 -o ../3filtered/$(basename $i .fastq)_filter.fastq
        done
```
...
for i in *R1*.fastq
        ...
        do
          	tagdust -ref /projects/crandall/MIGseq/Read2Seq.txt -dust 100 -1 R:N $i  -o ../dusted/$(basename $i .fastq)_dust
        done
...
for i in *R2*.fastq
        ...
        do
          	tagdust -ref /projects/crandall/MIGseq/Read1Seq.txt -dust 100 -1 R:N $i -o ../dusted/$(basename $i .fastq)_dust
        done
```
...
#Trim
echo "Running fastx_trimmer to remove msat bases from read 2"
cd 1original
../../fastq_trim.sh
#copy over R1 reads that weren't trimmed
cp *R1*.fastq ../2trimmed
cd ..
